...
test("maps reasoning, tool call deltas, usage, and finish reason from chat chunks", async (context) => {
  const gateway = await startGateway((_request, response) => {
    writeChunks(response, [
      ...
      '{"choices":[],"usage":{"prompt_tokens":120,"completion_tokens":30,"total_tokens":150,"prompt_tokens_details":{"cached_tokens":96}}}',
    ]);
  });
  context.after(() => gateway.close());
  ...
  assert.deepEqual(events, [
    ...
    { type: "usage", usage: { input: 120, output: 30, total: 150, cached: 96 } },
  ]);
});
