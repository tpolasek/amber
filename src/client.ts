...
interface Message { id: string; role: "user" | "assistant"; content: string; createdAt: string; status: "streaming" | "complete" | "error"; usage?: TokenUsage }
...
async function initialize(): Promise<void> {
  ...
  try {
    ...
    const id = location.pathname.match(/^\/s\/([a-f0-9-]{36})$/)?.[1];
...
function updateMessage(element: HTMLElement | null, message: Message): void {
  ...
  requiredWithin(element, ".role-name").textContent = message.role === "user" ? "OPERATOR" : "AGENT";
...
function scrollToBottom(smooth = true): void {
  requestAnimationFrame(() => elements.transcript.scrollTo({ top: elements.transcript.scrollHeight, behavior: smooth ? "smooth" : "auto" }));
}
