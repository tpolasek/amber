...
Agent types, selection guidance, system prompts, and read-only permissions come from the `agents` array. Each agent may also set its own `model`, `auth_key`, and `auth_url`; omitted values independently inherit the application defaults, including environment-variable overrides. The first entry is used when an Agent call omits `subagent_type`. Read-only agents receive only the `Bash`, `Grep`, and `Read` tools; other agents receive all child-agent tools. An absent or empty `agents` array disables the Agent tool.
...
- `src/grep-tool.ts` — the Grep tool runs ripgrep (`rg` must be on PATH) for content search with `content`, `files_with_matches`, and `count` output modes, context/line-number/case-insensitive/multiline flags, glob and type filters, and `head_limit`/`offset` pagination.
...
- `/commit` asks the model to review the working tree and create a commit with a concise, to-the-point title, bullet-point body, and the high-level details of the change. It expands to a full prompt in the transcript and does not push. `/commit push` also pushes the resulting commit to the remote; no other arguments are accepted.
...
- `/git diff`, `/git show`, and `/git status` open a modal that runs the equivalent read-only git command in the session CWD via `GET /api/sessions/:id/git?command=diff|show|status`. Diff output is rendered with the same diff highlighting as Write/Edit tool results; the diff view also offers Commit and Commit + Push buttons that trigger the `/commit` flow. `/git commit` is equivalent to `/commit`, and `/git commit push` to `/commit push`; no other subcommands or arguments are accepted.
...
- `POST /api/sessions/:id/plan-mode/:toolUseId/decision` resolves the matching pending entry or exit review with `{ "approved": boolean, "feedback"?: string, "cancelled"?: boolean }`. Interactive message streams emit `plan_mode_request` events while awaiting this decision.
...
- `ExitPlanMode` accepts an optional `allowedPrompts` array of `{ "tool": "Bash", "prompt": string }` entries and must also be the sole tool call. Amber validates that the saved plan is present, nonblank, and no more than 100,000 characters before showing a sanitized Markdown snapshot. Approval exits plan mode and returns that reviewed snapshot to the model for immediate implementation. “Keep Planning” requires feedback and returns it for immediate revision. Closing with X or `Esc` keeps plan mode active, ends the current model turn, and waits for another user prompt. Requested Bash categories are retained and displayed informationally only.
- Plan-mode enforcement is intentionally hybrid rather than a general permission framework. Amber hard-blocks its dedicated `Write` and `Edit` tools for every path except the active session’s plan file. `Read` remains available, and planning subagents receive only `Read`, `Bash`, and `Grep`. Bash is technically capable of mutation and follows the injected read-only instruction; `allowedPrompts` does not create command rules or alter Bash behavior.
...
- `Grep` searches file contents with ripgrep regexes through the system `rg` binary. It searches the session CWD by default, or a `path` (file or directory) resolved within the project and `/add-dir` roots. `output_mode` selects `content` (matching lines with `-n` line numbers and `-A`/`-B`/`-C`/`context` context), `files_with_matches` (default; sorted newest first), or `count`. `-i`, `multiline`, `glob`, and `type` map to the corresponding ripgrep flags. `head_limit` (default 250, `0` for unlimited) and `offset` paginate every mode. Matches outside the CWD keep absolute paths; exit code 1 (no matches) is a normal empty result.
